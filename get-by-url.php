...
// The external URL of the blog post
$url = $_POST['url'] ?? null;
...
class ContentFetcher
{
    public function run($url)
    {
        ...
        $entry = new \stdClass();
        $entry->title = $title;
        $entry->body = $this->clean($data['content']);

        return $data;
    }

    function clean($text, $config = null) {
    ...
    public function sanitize_text($string)
    {
        ...
        return $this->clean($string, 'titles');
    }
}

try {
    $data = (new ContentFetcher)->run($url);
    if(isset($data['preview']) && !empty($data['preview'])) {
        $img = file_get_contents($data['preview']);
        $file = time().'-image.jpg';
        file_put_contents('./content/uploads/'.$file, $img);
        $data['preview'] = $file;
    }
    $endContent = "<a href=".$_POST['url'].">Source | المصدر</a> <p>According to the Terms of Service, the person who republished the article on Polex undertakes that he owns the publishing rights or is involved in writing it to the source who published it for the first time.<br> حسب شروط الخدمة يتعهد من قام بإعادة نشر المقال في بولكس أنه يملك حقوق نشرها أو أنه مشارك في كتابتها للمصدر الأصل الذي نشر المقال أول مرة</p>";
    $data['content'] = $data['content'].''.$endContent;
    $user->post_article('timeline', null, null, null, $data['headline'], $data['content'], $data['preview'], $_POST['category'], null, $_POST['country'], $_POST['type']);
