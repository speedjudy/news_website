...
class User
{
  ...
  public function get_posts($args = [])
  {
    ...
    if ($system['newsfeed_location_filter_enabled'] && $country != "all") {
      $location_join = " LEFT JOIN users AS user_post_author ON posts.user_type = 'user' AND posts.user_id = user_post_author.user_id LEFT JOIN pages AS page_post_author ON posts.user_type = 'page' AND posts.user_id = page_post_author.page_id LEFT JOIN posts_articles ON posts_articles.post_id = posts.post_id ";
      ...
      $rawCountry = $_GET['country'];
      $where_query .= " AND ( (posts.user_type = 'user' AND posts_articles.country = '$rawCountry') OR (posts.user_type = 'page' AND page_post_author.page_country = '$country') )";
    }
  ...
  {
    global $db;
    $get_types = $db->query(sprintf("SELECT * FROM system_types")) or _error("SQL_ERROR_THROWEN");
    $types = [];
   ...
   * @return integer
   */
  public function post_article($publish_to, $page_id, $group_id, $event_id, $title, $text, $cover, $category_id, $tags, $country = null, $type = null)
  {
    global $db, $system, $date;
    ...
    /* insert article */
    $db->query(sprintf("INSERT INTO posts_articles (post_id, cover, title, text, category_id, tags, country, type_id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)", secure($post_id, 'int'), secure($cover), secure($title), secure($clean_text), secure($category_id, 'int'), secure($tags), secure($country, 'string'), secure($type, 'int'))) or _error("SQL_ERROR_THROWEN");
    /* points balance */
