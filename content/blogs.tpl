          ...
          <div class="js_ajax-forms-html" data-url="posts/article.php?do=create">
            <div class="card-body">
              ...
              <div class="form-group form-row">
                ...
                <div class="col-md-10">
                  <textarea name="text" class="form-control js_wysiwyg" id="texteditor"></textarea>
                </div>
              </div>
...
<script>
  ...
  if (window.location.search.includes('by_url')) {
    $('#get-article').on('click', function(e) {
      ...
      if(document.getElementById('by-url').checkValidity()) {
        $('#url-error').addClass('d-none');
        $.post('/get-by-url.php', data, function(response) {
          let data = JSON.parse(response);

          if('headline' in data) {
            $('input[name=title]').val(data.headline);
          }
          if('content' in data) {
            tinymce.get('texteditor').setContent(data.content);
          }
          if('preview' in data) {
            $('.x-image').css('background-image','url("/content/uploads/'+data.preview+'")');
            $('.js_x-image-input').val(data.preview);
          }
