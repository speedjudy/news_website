  ...
  <div class="publisher-slider">
    ...
    <div class="publisher-tools-tabs">
      ...
      <ul class="row">
        ...
        {/if}
         <li class="col-md-6">
            <a class="publisher-tools-tab link js_publisher-tab" id="triggerByURL" data-tab="article" href='{$system['system_url']}/blogs/new{if $_handle == "page"}?page={$_id}{/if}{if $_handle == "group"}?group={$_id}{/if}{if $_handle == "event"}?event={$_id}{/if}?by_url=1'>
              {include file='__svg_icons.tpl' icon="url" class="mr5" width="24px" height="24px"}
              {__("Publish by link")}
            </a>
          </li>
      ...
      <form id="byUrlWrapper" class="pb-2">
        <div class="form-group">
          <label class="form-control-label"> {__("URL")} </label>
          <input type="url" id="by-url" class="form-control" placeholder="{_('Enter the URL')}" required>
          <div class="text-danger mt-2 d-none" id="url-error">{_('Enter a valid URL')}</div>
        </div>
        <div class="row">
          <div class="form-group col-md-4">
            <label class="form-control-label"> {__("Types")} </label>
            <select class="form-control" name="type">
              <option value="" noselect>{__("Select Type")}</option>
          ...
          <div class="form-group  col-md-4">
            ...
            <select class="form-control" name="category" required>
                <option value="" noselect>{__("Select Category")}</option> {foreach $blogs_categories as $category} {include file='__categories.recursive_options.tpl' data_category=$article['article']['category_id']} {/foreach}
            </select>
          </div>
          <div class="form-group col-md-4">
            <label class="form-control-label"> {__("Country")} </label>
            <select class="form-control" name="country">
                <option value="" noselect>{__("Select Country")}</option> 
        ...
        <div class="d-flex justify-content-end">
          <button class="btn btn-primary" id="get-article" type="submit">{_('Get & Post Article')}</button>
        </div>
      </form>
    </div>
    <!-- publisher-tools-tabs -->
    ...
    <div class="publisher-footer">
      {if $user->_data['can_add_anonymous_posts'] && $_handle == "me"}
...
<!-- Modals -->
<div id="url-modal" class="modal fade">
<div class="modal-dialog">
  <div class="modal-content">
    <div class="modal-body">
      <div class="form-group">
        <input type="url" id="by-url" class="form-control" placeholder="{_('Enter the URL')}">
        <div class="text-danger mt-2 d-none" id="url-error">{_('Enter a valid URL')}</div>
      </div> 
      <button class="btn btn-primary" id="get-article">{_('Get article')}</button>
    </div>
  </div>
</div>
</div>
...
window.onload = function() {
  $('#byUrlWrapper').hide();
  $('#byUrlWrapper').on('submit', function(e) {
    e.preventDefault();
    $('#get-article').attr('disabled', 'disabled');
    ...
    $.post('/get-by-url.php', data, function(response) {
      let data = JSON.parse(response);
      if(data.length == 0) {
        $('#url-error').removeClass('d-none').html("Unfortunately, the feature you are trying to access is not supported by the website you have entered. Kindly visit the article and manually copy its content.");
      $('#get-article').removeAttr('disabled');
      }else {
        location.reload();
      }
    });
  });
  $('#triggerByURL:not(.disabled)').on('click', function(e) {
    e.preventDefault();
    if(!$(this).hasClass('disabled')) {
      $('.js_publisher-tab').toggleClass('disabled')
      $(this).removeClass('disabled')
      $('#byUrlWrapper').toggle();
      $('.js_publisher-btn').toggle();
    }
  });
}
